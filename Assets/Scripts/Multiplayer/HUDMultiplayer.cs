...
public class HUDMultiplayer : MonoBehaviour {
	...
	void Update()
	{
		...
		{
			float distance = generateLevel.levelLengthInMeters - ( localPlayerControl.tileDistanceTraveled + localPlayerRace.getDistanceTravelledOnThisTile() );
	...
	string getRacePositionsForAllPlayers()
	{
		...
		for( int i = 0; i < PlayerRace.players.Count; i++ )
		{
			...
			infoForAllPlayers.Append( PlayerRace.players[i].getDistanceTravelledOnThisTile().ToString( "N0" ) );
