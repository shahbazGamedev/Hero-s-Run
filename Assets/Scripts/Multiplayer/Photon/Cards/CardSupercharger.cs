...
public class CardSupercharger : Card {
	...
	void cardSuperchargerMasterRPC( int level, int photonViewID )
	{
		...
		playerTransform.GetComponent<PhotonView>().RPC("cardSuperchargerRPC", PhotonTargets.AllViaServer, cd.getCardPropertyValue( CardPropertyType.DURATION_WITH_TIMER, level ) );
	}
	#endregion
