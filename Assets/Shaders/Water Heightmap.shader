...
Shader "Water/Water Hightmap" {
	...
	Properties {
		...
		_ScrollUStrength ("Scroll U strength", Range(-5.0,5.0)) = 1.0
		_ScrollVStrength ("Scroll V strength", Range(-5.0,5.0)) = 0.0
	}
	...
	SubShader {
		...
		Pass {
			...
			fixed _ScrollUStrength;
			fixed _ScrollVStrength;
			...
			fixed4 frag(VertexToFragment i) : COLOR {
				...
				_HeightMap1ScrollU = _HeightMapStrength * _ScrollUStrength * _Time;
				_HeightMap1ScrollV = _HeightMapStrength * _ScrollVStrength * _Time;
				...
				// Background
  				fixed4 background1 = tex2D(_MainTex, float2(_MainTexTile,_MainTexTile)*i.uv+(_MainTexRefraction*(distortion1))+float2(_MainTexScrollU1,_MainTexScrollV1));
